...
/// [StickySlider] uses this callback to paint the value indicator on the overlay.
///
...
///
/// {@tool dartpad}
/// This sample shows the creation of a [StickySlider] using [ThemeData.useMaterial3] flag,
...
///
/// {@tool dartpad}
/// This example shows a [StickySlider] widget using the [StickySlider.secondaryTrackValue]
...
/// [max]. To use discrete values, use a non-null value for [points], which
/// indicates the number of discrete intervals. For example, if [min] is 0.0 and
/// [max] is 50.0 and [points] is 5, then the slider can take on the
...
class StickySlider extends StatefulWidget {
  ...
  /// Creates an adaptive [StickySlider] based on the target platform, following
  /// Material design's
  ...
  ///
  /// If not null, a secondary track using [StickySlider.secondaryActiveColor] color
  /// is drawn between the thumb and this value, over the inactive track.
  ///
  /// If less than [StickySlider.value], then the secondary track is not shown.
  ///
  /// It can be ideal for media scenarios such as showing the buffering progress
  /// while the [StickySlider.value] shows the play progress.
  ...
  /// the [StickySlider.secondaryTrackValue].
  ///
  ...
  ///
  /// If this color is null, [StickySlider] will use [activeColor], If [activeColor]
  /// is also null, [StickySlider] will use [SliderThemeData.thumbColor].
  ///
  ...
  ///
  /// If this property is null, [StickySlider] will use [activeColor] with
  ...
  @override
  State<StickySlider> createState() => _StickySliderState();
  ...
  void debugFillProperties(DiagnosticPropertiesBuilder properties) {
    ...
    properties.add(StringProperty('tracks', points.toString()));
    properties.add(DoubleProperty('trackCoverage', pointCoverage));
...
class _StickySliderState extends State<StickySlider> with TickerProviderStateMixin {
      ...
      child: FocusableActionDetector(
        ...
        child: CompositedTransformTarget(
          ...
          child: _SliderRenderObjectWidget(
            ...
            pointCoverage: widget.pointCoverage,
            label: widget.label,
...
class _SliderRenderObjectWidget extends LeafRenderObjectWidget {
  ...
  final ValueChanged<double>? onChangeEnd;
  final SemanticFormatterCallback? semanticFormatterCallback;
  final _StickySliderState state;
  final bool hasFocus;
  final bool hovering;
...
class _RenderSlider extends RenderBox with RelayoutWhenSystemFontsChangeMixin {
  _RenderSlider({
    ...
    required this.onChangeStart,
    required this.onChangeEnd,
    required _StickySliderState state,
  ...
  final _StickySliderState _state;
  ...
  bool get isDiscrete => points != null && points!.isNotEmpty;
  ...
  double? get pointCoverage => _pointCoverage;
  double? _pointCoverage;
  set pointCoverage(double? value) {
    if (value == _pointCoverage) {
      return;
    }
    _pointCoverage = value;
    markNeedsPaint();
  }
  ...
  void paint(PaintingContext context, Offset offset) {
    ...
    if (isDiscrete) {
      ...
      if (adjustedTrackWidth / points!.length >= 3.0 * tickMarkWidth) {
        final double dy = trackRect.center.dy;
        for (int i = 0; i < points!.length; i++) {
          final double value = points![i];
  ...
  double get _semanticActionUnit => points != null ? 1.0 / points!.length : _adjustmentUnit;
